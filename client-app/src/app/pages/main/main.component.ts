...
export class MainComponent implements OnInit, OnDestroy, AfterViewChecked {
  ...
  chatSelected(chatResponse: ChatResponse) {
    ...
    this.getAllChatMessages(chatResponse.id as unknown as string);
      ...
      reader.onload = () => {
        if (reader.result) {
          ...
          this.messageService.uploadMediaMessage({
            'chat-id': this.selectedChat.id as unknown as string,
            body: {
              file: file
            }
          }).subscribe({
  ...
  userProfile() {
    this.keycloakService.accountManagemant();
  }
    ...
    if (this.selectedChat && this.selectedChat.id === notification.chatId) {
      switch (notification.type) {
        ...
        case 'IMAGE':
          ...
          break;
        case 'SEEN':
          this.chatMessages.forEach(m => m.messageState = 'SEEN');
          break;
      }
    } else {
